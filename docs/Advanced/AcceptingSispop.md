title: Sispop.network Documentation | Accepting Sispop
description: This is a guide on how to accept Sispop cryptocurrency on your exchange using payment id's. This can be used for accepting sispop coin, a monero coin, in different services. This guide uses a cli wallet which was forked from xmr cli.

# Accepting Sispop 
## Instructions for the Command-Line Interface
### The Basics
Sispop works a little differently to what you may have become accustomed to from other cryptocurrencies. In the case of a digital currency like Bitcoin and its many derivatives merchant payment systems will usually create a new recipient address for each payment or user.

However, because Sispop has stealth addresses there is no need to have separate recipient addresses for each payment or user, and a single account address can be published. Instead, when receiving payments a merchant will provide the person paying with a `payment ID`.
...
To generate a payment ID in `sispop-wallet-cli` run the command:

```
payment_id
```

### Checking for a Payment in sispop-wallet-cli
If you want to check for a payment using `sispop-wallet-cli` you can use the `payments` command followed by the payment ID or payment IDs you want to check:
```
...
- Communicate the payment ID and Sispop address to the individual who is making payment

- Check for the payment using the "payments" command in sispop-wallet-cli
...
It is important to note that the amounts returned are in base Sispop units and not in the display units normally used in end-user applications. Also, since a transaction will typically have multiple outputs that add up to the total required for the payment, the amounts should be grouped by the `tx_hash` or the `payment_id` and added together. Additionally, as multiple outputs can have the same amount, it is imperative not to try and filter out the returned data from a single `get_bulk_payments` call.
