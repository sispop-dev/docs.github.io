title: Sispop.network Documentation | Sispop Daemon overview | sispopd
description: The Sispop coin daemon, forked from monerod, keeps your computer synced up with the Sispop network. It downloads and validates the blockchain from the p2p network. The daemon is also referenced as the nodes software.

# `sispopd` - Reference

## Overview

### Connects you to Sispop network

The Sispop daemon `sispopd` keeps your computer synced up with the Sispop network.

It downloads and validates the blockchain from the p2p network.

### Not aware of your private keys

`sispopd` is entirely decoupled from your wallet.

`sispopd` does not access your private keys - it is not aware of your transactions and balance.

This allows you to run `sispopd` on a separate computer or in the cloud.

In fact, you can connect to a remote `sispopd` instance provided by a semi-trusted 3rd party. Such 3rd party will not be able to steal your funds. This is very handy for learning and experimentation.
...
Go to directory where you unpacked Sispop.

The stagenet is what your should be using for learning and experimentation.

```
./sispopd --stagenet --detach                # run as a daemon in background
tail -f ~/.sispop/stagenet/sispop.log  # watch the logs
./sispopd --stagenet exit                    # ask daemon to exit gracefully
```

The mainnnet is when you want to deal with the real $SISPOP.

```
./sispopd --detach                           # run as a daemon in background
tail -f ~/.sispop/sispop.log           # watch the logs
./sispopd exit                               # ask daemon to exit gracefully
```
...
| `--version`         | Show `sispopd` version to stdout. Example: <br /> ` Sispop 'Festive Freya' (v2.0.2-68397db6)`
...
| Option           | Description                                                                                    
|------------------|------------------------------------------------------------------------------------------------
| (missing)        | By default `sispopd` assumes mainnet                                              
...
`sispopd` defaults are adjusted for running it occasionally on the same computer as your Sispop wallet.
...
| Option              | Description
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `--config-file`     | Full path to the configuration file. By default `sispop` looks for `sispop.conf` in Sispop data directory.
...
| `--pidfile`         | Full path to the PID file. Works only with `--detach`. Example: <br />`./sispopd --detach --pidfile=/run/sispop/sispopd.pid`
| `--detach`          | Go to background (decouple from the terminal). This is useful for long-running / server scenarios. Typically, you will also want to manage `sispopd` daemon with systemd or similar. By default `sispopd` runs in a foreground.
| `--non-interactive` | Do not require tty in a foreground mode. Helpful when running in a container. By default `sispopd` runs in a foreground and opens stdin for reading. This breaks containerization because no tty gets assigned and `sispopd` process crashes. You can make it run in a background with `--detach` but this is inconvenient in a containerized environment because the canonical usage is that the container waits on the main process to exist (forking makes things more complicated).
...
The following options define how your node participates in Sispop peer-to-peer network.
...
| Option                 | Description
|------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `--p2p-bind-ip`        | Network interface to bind to for p2p network protocol. Default value `0.0.0.0` binds to all network interfaces. This is typically what you want. <br /><br />You must change this if you want to constrain binding, for example to configure connection through Tor via torsocks: <br />`DNS_PUBLIC=tcp://1.1.1.1 TORSOCKS_ALLOW_INBOUND=1 torsocks ./sispopd --p2p-bind-ip 127.0.0.1 --no-igd --hide-my-port`
| `--p2p-bind-port`      | TCP port to listen for p2p network connections. Defaults to `20000` for mainnet, `38156` for testnet, and `38153` for stagenet. You normally wouldn't change that. This is helpful to run several nodes on your machine to simulate private Sispop p2p network (likely using private Testnet). Example: <br/>`./sispopd --p2p-bind-port=48080`
| `--p2p-bind-port-ipv6` | TCP port to listen for p2p network connections. Defaults to `20000` for mainnet, `38156` for testnet, and `38153` for stagenet.|
| `--p2p-bind-ipv6-address`|  Interface for p2p network protocol. |
| `--p2p-use-ipv6` | Enable IPv6 for p2p. |
| `--p2p-external-port`  | TCP port to listen for p2p network connections on your router. Relevant if you are behind a NAT and still want to accept incoming connections. You must then set this to relevant port on your router. This is to let `sispopd` know what to advertise on the network. Default is `0`.
| `--hide-my-port`       | `sispopd` will still open and listen on the p2p port. However, it will not announce itself as a peer list candidate. Technically, it will return port `0` in a response to p2p handshake (`node_data.my_port = 0` in `get_local_node_data` function). In effect nodes you connect to won't spread your IP to other nodes. To sum up, it is not really hiding, it is more like "do not advertise".
| `--seed-node`          | Connect to a node to retrieve other nodes' addresses, and disconnect. If not specified, `sispopd` will use hardcoded seed nodes on the first run, and peers cached on disk on subsequent runs.  
| `--add-peer`           | Manually add node to local peer list.
| `--add-priority-node`  | Specify list of nodes to connect to and then attempt to keep the connection open. <br /><br />To add multiple nodes use the option several times. Example: <br />`./sispopd --add-priority-node=178.128.192.138:18081 --add-priority-node=144.76.202.167:18081`
| `--add-exclusive-node` | Specify list of nodes to connect to only. If this option is given the options `--add-priority-node` and `--seed-node` are ignored. <br /><br />To add multiple nodes use the option several times. Example: <br />`./sispopd --add-exclusive-node=178.128.192.138:18081 --add-exclusive-node=144.76.202.167:18081`
...
| `--service-node`       | Generate a Service Node pubkey that identifies your Node to the network. This flag allows the ability to stake Sispop

#### Node RPC API

`sispopd` node offers powerful API. It serves 3 purposes:
...
| `--rpc-bind-port`               | TCP port to listen on. By default `30000` (mainnet), `38157` (testnet), `38154` (stagenet).
...
| `--rpc-access-control-origins`  | Specify a comma separated list of origins to allow cross origin resource sharing. This is useful if you want to use `sispopd` API directly from a web browser via JavaScript (say in a pure-fronted web appp scenario). With this option `sispopd` will put proper HTTP CORS headers to its responses. You will also need to set `--rpc-login` if you use this option. Normally though, the API is used by backend app and this option isn't necessary.
...
| Option           | Description
|------------------|------------------------------------------------------------------------------------------------
| `--block-notify` | Run a program for each new block. The argument must be a **full path**. If the argument contains `%s` it will be replaced by the block hash. Example: <br />`./sispopd --block-notify="/usr/bin/echo %s"`<br /><br />Couple of notes:<br />1) Block notifications are good for immediate reaction. However, you should always assume you will miss some block notifications and you should independently poll the API to cover this up.<br />2) Mind blockchain reorganizations. Block notifications can revert to same and past heights. This actually happens pretty often.<br />3) See also `--tx-notify` option of `sispop-wallet-rpc` daemon [here](https://github.com/monero-project/monero/pull/4333). 

#### Performance

These are advanced options that allow you to optimize performance of your `sispopd` node, sometimes at the expense of reliability.

| Option                          | Description
|---------------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `--db-sync-mode`                | Specify sync option, using format:<br />`[safe|fast|fastest]:[sync|async]:[<nblocks_per_sync>[blocks]|<nbytes_per_sync>[bytes]]`<br /><br />The default is `fast:async:250000000bytes`.<br /><br />The `fast:async:*` can corrupt blockchain database in case of a system crash. It should not corrupt if just `sispopd` crashes. If you are concerned with system crashes use `safe:sync`.
| `--max-concurrency`             | Max number of threads to use for a parallel jobs. The default value `0` uses the number of CPU threads.
| `--prep-blocks-threads`         | Max number of threads to use when computing block hashes (PoW) in groups. Defaults to 4. Decrease this if you don't want `sispopd` hog your computer when syncing.
| `--fast-block-sync`             | Sync up most of the way by using embedded, "known" block hashes. Pass `1` to turn on and `0` to turn off. This is on (`1`) by default. Normally, for every block the full node must calculate the block hash to verify miner's proof of work. Because the CryptoNight PoW used in Sispop is very expensive (even for verification), `sispopd` offers skipping these calculations for old blocks. In other words, it's a mechanism to trust `sispopd` binary regarding old blocks' PoW validity, to sync up faster.
| `--block-sync-size`             | How many blocks are processed in a single batch during chain synchronization. By default this is 20 blocks for newer history and 100 blocks for older history ("pre v4"). Default behavior is represented by value `0`. Intuitively, the more resources you have, the bigger batch size you may want to try out. Example:<br />`./sispopd --block-sync-size=500`
| `--bootstrap-daemon-address`    | The host:port of a "bootstrap" remote open node that the connected wallets can use while this node is still not fully synced. Example:<br/>`./sispopd --bootstrap-daemon-address=opennode.xmr-tw.org:18089`. The node will forward selected RPC calls to the bootstrap node. The wallet will handle this automatically and transparently. Obviously, such bootstraping phase has privacy implications similar to directly using a remote node.
...
The following options configure **solo mining** using **CPU** with the standard software stack `sispopd`. This is mostly useful for:
...
#### Testing Sispop itself

These options are useful for Sispop project developers and testers. Normal users shouldn't be concerned with these.
...
| `--save-graph`                     | Save data for dr Sispop.

#### Legacy

These options should no longer be necessary. They are still present in `sispopd` for backwards compatibility.
...
You can also type commands directly in the console of the running `sispopd` (if not detached). 
...
| `version`                          | Show version information. Example output:<br />`Sispop 'Festive Freya' (v2.0.2-68397db6)`
...
| `print_pl_stats`                   | Show the full peer list statistics (white vs gray peers). White peers are online and reachable. Grey peers are offline but your `sispopd` remembers them from past sessions.
| `print_cn`                         | Show connected peers with connection initiative (incoming/outgoing) and other stats.
| `ban <IP> [<seconds>]`             | Ban a given `<IP>` for a given amount of `<seconds>`. By default the ban is for 24h. Example:<br />`./sispopd ban 187.63.135.161`.
...
| Option                                                     | Description
|------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `print_coinbase_tx_sum <start_height> [<block_count>]`     | Show a sum of all emitted coins and paid fees within specified range. Example:<br />`./sispopd print_coinbase_tx_sum 0 1000000000000`
...
| Option                                                     | Description
|------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `show_hr`                                                  | Ask `sispopd` daemon to stop printing current hash rate. Relevant only if `sispopd` is mining. 
| `hide_hr`                                                  | Ask `sispopd` daemon to print current hash rate. Relevant only if `sispopd` is mining.
| `start_mining <addr> [<threads>] [do_background_mining] [ignore_battery]`   | Ask `sispopd` daemon to start mining. Block reward will go to `<addr>`.
| `stop_mining`                                              | Ask `sispopd` daemon to stop mining.

#### Testing Sispop itself

| Option                                                     | Description
|------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `start_save_graph`                                         | Start saving data for dr Sispop.
| `stop_save_graph`                                          | Stop saving data for dr Sispop.

#### Legacy

| Option                                                     | Description
|------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------
| `save`                                                     | Flush blockchain data to disk. This is normally no longer necessary as `sispopd` saves the blockchain automatically on exit. 
