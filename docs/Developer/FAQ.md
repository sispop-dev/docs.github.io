title: Sispop Documentation | Sispop Developer FAQ
description: See the FAQ questions and answers for Sispop Hardforks and changes. See information on Bulletproofs, Service Nodes and Hardfork changes.

# FAQ
## Hardfork Version 10: Bulletproofs
### `get_block_template` Updates
Due to batching of Governance rewards the calculation of rewards has been updated in Sispop. As a side effect of this, in the `get_block_template` json response, `expected_reward` now returns only the miner reward instead of the entire block reward.
