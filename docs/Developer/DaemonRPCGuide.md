title: Sispop Documentation | Sispop and Monero Daemon RPC Guide.
description: This is a list of the sispopd daemon RPC calls, their inputs and outputs, and examples of each. This RPC guide can be used by coins who share a codebase with Sispop Monero.

# Sispop Daemon RPC

## Introduction

This is a list of the sispopd daemon RPC calls, their inputs and outputs, and examples of each.

Many RPC calls use the daemon's JSON RPC interface while others use their own interfaces, as demonstrated below.

Note: "atomic units" refer to the smallest fraction of 1 SISPOP according to the sispopd implementation.  **1 SISPOP = 1e12 atomic units.**
...
The majority of sispopd RPC calls use the daemon's  `json_rpc`  interface to request various bits of information. These methods all follow a similar structure, for example:
...
-   _block_header_  - A structure containing block header information.
    -   _block_size_  - unsigned int; The block size in bytes.
    -   _depth_  - unsigned int; The number of blocks succeeding this block on the blockchain. A larger number means an older block.
    -   _difficulty_  - unsigned int; The strength of the Sispop network based on mining power.
    -   _hash_  - string; The hash of this block.
    -   _height_  - unsigned int; The number of blocks preceding this block on the blockchain.
    -   _major_version_  - unsigned int; The major version of the sispop protocol at this block height.
    -   _minor_version_  - unsigned int; The minor version of the sispop protocol at this block height.
    -   _nonce_  - unsigned int; a cryptographic random one-time number used in mining a Sispop block.
    ...
    -   _reward_  - unsigned int; The amount of new    generated in this block and rewarded to the miner. Note: 1 SISPOP = 1e12  atomic units.
...
-   _block_size_limit_  - unsigned int; Maximum allowed block size
-   _block_size_median_  - unsigned int; Median block size of latest 100 blocks
-   _bootstrap_daemon_address_  - string;  [bootstrap node](https://ww.getsispop.org/resources/sispoppedia/bootstrap-node.html)  to give immediate usability to wallets while syncing by proxying RPC to it. (Note: the replies may be untrustworthy).
-   _cumulative_difficulty_  - unsigned int; Cumulative difficulty of all blocks in the blockchain.
...
-   _emission_amount_  - unsigned int; amount of coinbase reward in  atomic units
-   _fee_amount_  - unsigned int; amount of fees in  atomic units
-   _status_  - string; General RPC error code. "OK" means everything looks good.
...
-   _status_  - string; General RPC error code. "OK" means everything looks good.
-   _target_height_  - unsigned int; target height the node is syncing from (optional, absent if node is fully synced)
    ...
    -   _as_json_  - json string; List of transaction info:
        ...
        -   _vin_  - List of inputs into transaction:
            -   _key_  - The public key of the previous output spent in this transaction.
                ...
                -   _key_offsets_  - A list of integer offets to the input.
...
-   _tx_as_hex_  - string; Full transaction information as hexidecimal string.
...
-   _ignore_battery_  - boolean; States if batery state (on laptop) should be ignored (`true`) or not (`false`).
...
-   _gray_list_  - array of offline  _peer_  structure as follows:
    ...
    -   _port_  - unsigned int; TCP port the peer is using to connect to sispop.site.
-   _status_  - string; General RPC error code. "OK" means everything looks good. Any other value means that something went wrong.
...
-   _transactions_  - List of transactions in the mempool are not in a block on the main chain at the moment:
    ...
    -   _last_failed_height_  - unsigned int; If the transaction validation has previously failed, this tells at what height that occured.
    ...
    -   _max_used_block_hash_  - string; Tells the hash of the most recent block with an output used in this transaction.
    ...
    -   _tx_blob_  - unsigned int; Hexadecimal blob represnting the transaction.
    -   _tx_json_  - json string; JSON structure of all information in the transaction:
        ...
        -   _rct_signatures_  - Ring signatures:
            -   _type_
            -   _txnFee_
...
## Sources: 
Reworked from [GetMonero.org](https://ww.getmonero.org/resources/developer-guides/daemon-rpc.html) RPC calls for Sispop under their [copyright license](https://ww.getmonero.org/legal/#copyright).