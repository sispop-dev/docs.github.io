title: Sispop Documentation | 2 of 3 Multisig Wallet Guide
description: Do you need to setup a multisig wallet on the Sispop or Monero Blockchain? Here is a guide provided by Sispop Network to get you setup with a 2 of 3 Multisignature Wallet.
...
-   3 empty `sispop-wallet-cli` wallets
...
All 3 people should open up their `sispop-wallet-cli` and generate a new wallet. Make sure you do not have any $sispop within your wallet.

**Person 1, 2** and **3** runs the following command within their `sispop-wallet-cli`:
...
Without this step, it will not be possible to create a transaction that spends Sispop. As a minimum, the sender needs to get a partial key image from the same person who will sign the transaction with him later. He could get from both parties immediately and then later decide with whom to sign.
...
**Person 1** should change to the directory of their `sispop-wallet-cli` and use Person 2 and 3’s download link to run the commands:
...
Likewise, Person 2 and 3 should do the same, changing directories to their `sispop-wallet-cli` and downloading with the alternative Persons download link, and filename.
...
Send the file `multisig_sispop_tx` to either person 1 or 2.

**Person 3** will send the file `multisig_sispop_tx` to the Person 1 or 2. **Person 3** can send this file through email or alternatively use the transfer.sh commands outside of the wallet:
...
Replacing `https://transfer.sh/CJqnM/multisig_sispop_tx` with the link provided by Person 3.

### Step 2 - Sign Multisig

Let's say Person 2 was picked as the partner. He must finish the signature. Person 2 copies the file to the same folder from where he started (or will start) `sispop-wallet-cli`.
